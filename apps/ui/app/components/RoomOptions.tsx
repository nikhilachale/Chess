...
export default function RoomOptions({ socket, player }: Props) {
  ...
  const [activeRooms, setActiveRooms] = useState(8);

  // Simulate live stats updates
  useEffect(() => {
    const interval = setInterval(() => {
      setPlayerCount(prev => prev + Math.floor(Math.random() * 3) - 1);
      setActiveRooms(prev => Math.max(1, prev + Math.floor(Math.random() * 3) - 1));
    }, 3000);
  ...
  return (
    <div className="relative w-full min-h-screen overflow-hidden">
      ...
      {/* Content */}
      <div className="relative z-10 flex items-center justify-center min-h-screen px-6 pt-8">
        <div className="text-center space-y-12 max-w-4xl animate-fade-in-up">
          ...
          <div className="space-y-6">
            ...
            <h1 className="text-5xl md:text-6xl font-extrabold text-stone-100 leading-tight">
              Welcome to the <br/>
            ...
            <p className="text-stone-300 text-xl leading-relaxed max-w-2xl mx-auto">
              Greetings, <span className="font-bold text-amber-200">{player?.username}</span>! 
              Enter the sacred halls where chess masters gather to battle for glory.
            </p>
          </div>
          ...
          <div className="bg-gradient-to-br from-stone-900/70 to-zinc-900/70 backdrop-blur-xl rounded-3xl p-10 border-2 border-amber-600/40 shadow-2xl">
            <div className="space-y-8">
              <div className="text-center">
                ...
                <p className="text-stone-400 text-lg">Begin your quest for chess mastery</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <button 
                  ...
                  className="group relative px-8 py-6 bg-gradient-to-r from-amber-600 via-yellow-600 to-amber-500 hover:from-amber-700 hover:via-yellow-700 hover:to-amber-600 text-zinc-900 rounded-2xl font-bold transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-amber-500/50 border-2 border-amber-400/50 hover:border-amber-300/60 disabled:from-stone-600 disabled:to-stone-700 disabled:text-stone-400 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none"
                >
                  <span className="flex items-center justify-center gap-3 text-lg">
                    {roomChoice === "join" ? (
                      <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-stone-800/40 backdrop-blur-xl rounded-2xl p-6 border border-amber-600/30 shadow-lg text-center">
              ...
              <div className="text-stone-400 text-sm">Active Knights</div>
            </div>
            <div className="bg-stone-800/40 backdrop-blur-xl rounded-2xl p-6 border border-amber-600/30 shadow-lg text-center">
              <div className="text-3xl mb-2">🏰</div>
              <div className="text-amber-400 font-bold text-lg">{activeRooms}</div>
              <div className="text-stone-400 text-sm">Game Rooms</div>
            </div>
            <div className="bg-stone-800/40 backdrop-blur-xl rounded-2xl p-6 border border-amber-600/30 shadow-lg text-center">
              <div className="text-3xl mb-2">⚡</div>
              <div className="text-amber-400 font-bold text-lg">{socket ? 'Ready' : 'Offline'}</div>
              <div className="text-stone-400 text-sm">Connection</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {showJoinPopup && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gradient-to-br from-stone-900/95 to-zinc-900/95 backdrop-blur-xl rounded-3xl p-8 w-full max-w-md shadow-2xl border-2 border-amber-600/40 animate-fade-in-up">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-stone-400">
                Enter the chamber code to join an existing battle
              </p>
            </div>
            ...
            <div className="space-y-6">
              <div className="relative">
                <input
                  type="text"
                  value={joinRoomName}
                  onChange={(e) => setJoinRoomName(e.target.value.toUpperCase())}
                  className="w-full px-6 py-4 bg-stone-800/60 backdrop-blur-xl border-2 border-amber-700/50 rounded-2xl text-amber-100 placeholder-stone-400 focus:border-amber-500 focus:outline-none focus:ring-4 focus:ring-amber-500/30 transition-all duration-300 text-lg font-semibold text-center tracking-wide"
                  placeholder="Enter chamber code..."
                  onKeyDown={(e) => e.key === "Enter" && handleConfirmJoinRoom()}
                  autoFocus
                />
              ...
              <div className="grid grid-cols-2 gap-4">
                <button
                  ...
                  className="group px-6 py-4 bg-stone-700/60 hover:bg-stone-600/70 text-stone-200 rounded-2xl font-bold transition-all duration-300 border border-stone-600/50 hover:border-stone-500/60"
                >
                ...
                <button
                  ...
                  className="group px-6 py-4 bg-gradient-to-r from-amber-600 to-yellow-600 hover:from-amber-700 hover:to-yellow-700 text-zinc-900 rounded-2xl font-bold disabled:from-stone-600 disabled:to-stone-700 disabled:text-stone-400 disabled:cursor-not-allowed transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-amber-500/50 border border-amber-400/50 hover:border-amber-300/60 disabled:transform-none disabled:shadow-none"
                >
                  <span className="flex items-center justify-center gap-2">
              ...
              <div className="border-t border-amber-700/30 pt-4">
                <p className="text-stone-500 text-xs mb-3 text-center">Suggested chambers:</p>
                <div className="flex flex-wrap justify-center gap-2">
                  {['ROYAL001', 'MASTER1', 'KNIGHT7'].map((code) => (
                    <button
                      key={code}
                      onClick={() => setJoinRoomName(code)}
                      className="px-3 py-1 bg-stone-800/40 hover:bg-amber-600/20 text-stone-300 hover:text-amber-200 rounded text-xs font-mono transition-colors duration-300 border border-stone-600/30 hover:border-amber-600/40"
                    >
                      {code}
                    </button>
                  ))}
